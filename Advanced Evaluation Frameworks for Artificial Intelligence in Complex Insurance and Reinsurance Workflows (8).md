...
### **Example 3: Pension Actuarial (ERISA 430(h) Segment Rate Corridor & Floor Trap)**
...
"You are an Enrolled Actuary. The current date is January 15, 2026\. You are performing the minimum funding valuation for a single-employer defined benefit pension plan for the 2026 plan year. The plan sponsor has elected to use the September 2025 segment rates to discount the plan's liabilities.

Under the ARP/IIJA stabilized segment rate rules (IRC 430(h)(2)(C)(iv)), the unadjusted 24-month average segment rates must fall within a permissible statutory corridor of the 25-year average segment rates. The statutory corridor for the 2026 plan year is 95% to 105%. Furthermore, the 25-year averages are subject to a strict statutory absolute floor of 5.00% before the corridor is calculated.

Review the attached segment\_rate\_data.csv. Draft an Actuarial Certification Memo. You must:
...
**Attached Files (Context):** segment\_rate\_data.csv

**Reference URL:** [https://www.irs.gov/retirement-plans/pension-plan-funding-segment-rates](https://www.irs.gov/retirement-plans/pension-plan-funding-segment-rates)"

**3\. Rubric (Negative Failure Focus)**

* **Extracts data for September 2025 (Critical):** Must bypass decoys and pull 6 specific floats for Sept 2025\. *Failure:* Pulled Oct 2025 data.  
* **Applies 5.00% Floor (Critical):** Must elevate 3.22% Seg 1 Raw Average to exactly 5.00%. Seg 2 (5.00%) and Seg 3 (5.70%) remain unchanged. *Failure:* Applied floor to Unadjusted 24-Month rates.  
* **Calculates Corridor Bounds (Major):** Seg 1: 4.75% to 5.25%. Seg 2: 4.75% to 5.25%. Seg 3: 5.42% to 5.99%. *Failure:* Calculated Seg 1 based on raw 3.22%.  
* **Determines Final Seg 1 Rate (Critical):** 4.81% falls inside the 4.75%–5.25% corridor. Final \= 4.81%.  
* **Determines Final Seg 2 Rate (Critical):** 5.35% exceeds the 5.25% corridor maximum. Final \= 5.25% (Capped).  
* **Determines Final Seg 3 Rate (Critical):** 5.69% falls inside the 5.42%–5.99% corridor. Final \= 5.69%.  
* **Recommendation (Minor):** Lists the final triad: 4.81%, 5.25%, and 5.69%.
...
**1\. CSV Data Extraction:** Raw 25-Year Averages: Seg 1: 3.22%, Seg 2: 5.00%, Seg 3: 5.70%. Unadjusted 24-Month Averages: Seg 1: 4.81%, Seg 2: 5.35%, Seg 3: 5.69%.

**2\. Application of the Statutory Floor:** Under ARP/IIJA, the 25-year average segment rates are subject to an absolute statutory floor of 5.00%. Segment 1 is increased to 5.00%. Segment 2 remains 5.00%. Segment 3 remains 5.70%.

**3\. Corridor Calculation:** 95% to 105% of the floored averages: Seg 1 Corridor (4.75% to 5.25%), Seg 2 Corridor (4.75% to 5.25%), Seg 3 Corridor (5.42% to 5.99%).

**4\. Final Segment Rates:** Seg 1 unadjusted rate is 4.81% (no adjustment needed). Seg 2 unadjusted is 5.35% (exceeds maximum, capped at 5.25%). Seg 3 unadjusted is 5.69% (no adjustment needed).

**5\. Actionable Recommendation:** I certify the use of the following stabilized segment rates: Segment 1: 4.81%, Segment 2: 5.25%, Segment 3: 5.69%."

### ---

**Example 4: Life Actuarial (IRC 7702(f)(7) "Years 6-15" Recapture Ceiling Trap)**
...
"You are a Life Insurance Pricing Actuary. The current date is October 15, 2025\. You are testing an in-force Universal Life block for compliance under IRC Section 7702 and 7702A. A policyholder recently requested a severe reduction in their death benefit face amount, which was executed on September 1, 2025\. To force the policy back into compliance with the statutory limits, a cash distribution was made to the policyholder.

Because this change reduced benefits and resulted in a cash distribution, you must calculate the exact 'Recapture Ceiling' under the IRC Section 7702(f)(7) rules to determine the federally taxable portion of the distribution. The policy being reviewed is UL-992B. It was originally issued on June 1, 2015\. It is tested strictly under the Guideline Premium Test (GPT).

Review the attached ul\_policy\_transactions.csv. Draft a Tax Compliance Memo. You must:

* Extract the pre- and post-transaction floating-point financial values specifically for UL-992B. Beware of decoy policies.  
* Determine the exact policy age (in years) at the time of the reduction.  
* Conduct a statutory analysis of IRC Section 7702(f)(7) to determine whether the "Years 1-5" rule or the "Years 6-15" rule applies based on the policy age.  
* Define the exact statutory formula required for your determined time period under a GPT policy.  
* Execute the exact floating-point calculation to establish the Recapture Ceiling, mathematically isolating only the relevant variables and ignoring the decoys. State the final dollar amount.

**Attached Files (Context):** ul\_policy\_transactions.csv

**Reference URL:** [https://www.law.cornell.edu/uscode/text/26/7702](https://www.law.cornell.edu/uscode/text/26/7702)"

**3\. Rubric (Negative Failure Focus)**

* **Extracts data for UL-992B (Minor):** Must correctly isolate UL-992B. *Failure:* Extracted UL-992A.  
* **Calculates Policy Age (Major):** Time from June 1, 2015, to September 1, 2025, is exactly 10 years and 3 months (Policy Year 11).  
* **Identifies "Years 6-15" Rule (Critical):** Because the policy is in Year 11, it falls under the IRC 7702(f)(7)(D) rule.  
* **Identifies GPT Formula (Critical):** For Years 6-15, the Recapture Ceiling is the Cash Surrender Value (CSV) immediately before reduction MINUS the Cash Value Corridor (CVC) immediately after reduction. *Failure:* Used GLP or NSP variables.  
* **Calculates Recapture Ceiling (Critical):** Must subtract the CVC\_After ($265,400.75) from the CSV\_Before ($315,650.25) to equal exactly $50,249.50.
...
**1\. CSV Data Extraction:** According to the ul\_policy\_transactions.csv, the relevant financial values for policy UL-992B... CSV Before Reduction: $315,650.25, CVC After Reduction: $265,400.75.

**2\. Policy Age Determination:** The policy was issued on June 1, 2015, and the reduction occurred on September 1, 2025\. This means the policy was exactly 10 years and 3 months old at the time of the reduction (in Policy Year 11).

**3\. Statutory Application:** Because the reduction occurred in Policy Year 11, the "Years 6-15" rule under IRC 7702(f)(7)(D) applies.

**4\. Formula:** For a reduction during Years 6-15, the Recapture Ceiling formula is the Cash Surrender Value (CSV) immediately before the reduction minus the Cash Value Corridor (CVC) determined immediately after the reduction. The NSP and GLP variables are statutory decoys.

**5\. Final Calculation:** $315,650.25 \- $265,400.75 \= $50,249.50. The statutory Recapture Ceiling for UL-992B is exactly $50,249.50."
