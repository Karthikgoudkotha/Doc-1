...
* Role: You are a Senior U.S. Commercial Lines Underwriter (SME Property/BOP).  
* Context:   
* You are putting together a quick summary for a morning referral huddle based on a broker submission for a multi-location account. The policy effective date is 01/01/2024. You must underwrite strictly using the attached "underwriting\_guidelines.pdf" and "submission\_data.csv". Do not invent rules or use outside knowledge.  
...
* 4\. Final Premium \= Apply Section IV Minimum Premium rule ($500) if applicable, then round to the nearest whole dollar.  
...
### **Why this version fixes the Evaluation Framework errors:**

* **Fixing "Criterion Atomicity":** By breaking the prompt into four distinct "Tasks" with rigid output formatting, the LLM stops blending its qualitative underwriting thoughts with its quantitative pricing math. An automated evaluation script (or a human grader) can now look at Task 2 *only* for rules logic, and Task 3 *only* for math.  
* **Fixing "Criterion Comprehensiveness":** Standard LLMs suffer from "output truncation" or laziness when dealing with CSV data. By explicitly stating "Evaluate ALL locations (Loc\_1 through Loc\_5)" and "ALL 5 locations", you set a hard boundary. If the LLM stops at Loc\_4, it automatically fails the evaluation.  
* **Fixing "Criterion Factual and Mathematical Accuracy":** The original prompt said "Calculate the indicated premium... using the rating method in the PDF." LLMs are notoriously bad at implicit math. The tuned prompt defines the exact equations step-by-step (Base Premium \= ...) and gives strict rules for floating-point handling ("Do not round the intermediate multiplier" and "round to the nearest whole dollar"). This ensures the math is 100% reproducible and verifiable by your evaluation rubric.

### **Quick Reference: The Expected Ground Truth**

To help you build your evaluation rubric, here is the mathematically accurate data the prompt should generate based on the images you provided:

* **Loc\_1 (Office):** Accept. Base Premium: $1,250. Multiplier: 0.6538. Indicated: $817. Final: **$817**.  
* **Loc\_2 (Retail):** Refer (Rule P.1, Rule L.1). Base: $1,200. Multiplier: 1.5939. Indicated: $1,913. Final: **$1,913**.  
* **Loc\_3 (Manufacturing):** Decline (Rule O.1). Base: $10,200. Multiplier: 0.8925. Indicated: $9,104. Final: **$9,104** *(Excluded from total)*.  
* **Loc\_4 (Restaurant):** Decline (Rule D.1 \- 1965 is pre-1970). Base: $8,800. Multiplier: 0.9448. Indicated: $8,314. Final: **$8,314** *(Excluded from total)*.  
* **Loc\_5 (Office):** Accept. Base: $375. Multiplier: 0.6538. Indicated: $245. Final: **$500** *(Minimum premium applied)*.

**Total Account Premium:** $817 \+ $1,913 \+ $500 \= **$3,230**.
